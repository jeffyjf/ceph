...
#include "ImageTempLocalWriteback.h"
...
#undef dout_prefix
#define dout_prefix *_dout << "librbd::ImageTempLocalWriteback: " << __func__ << ": "

namespace librbd {
namespace cache {

template <typename I>
ImageTempLocalWriteback<I>::ImageTempLocalWriteback(I &image_ctx) : m_image_ctx(image_ctx) {
}

template <typename I>
void ImageTempLocalWriteback<I>::aio_read(Extents &&image_extents, bufferlist *bl,
  ...
  auto req = io::ImageDispatchSpec::create_read(
    *image_ctx, io::IMAGE_DISPATCH_LAYER_TEMP_LOCAL_CACHE, aio_comp,
...
template <typename I>
void ImageTempLocalWriteback<I>::aio_write(Extents &&image_extents,
                                  ceph::bufferlist&& bl,
                                  int fadvise_flags, Context *on_finish) {
  CephContext *cct = m_image_ctx.cct;
  ldout(cct, 20) << "image_extents=" << image_extents << ", "
...
template <typename I>
void ImageTempLocalWriteback<I>::aio_discard(uint64_t offset, uint64_t length,
  ...
  auto req = io::ImageDispatchSpec::create_discard(
    *image_ctx, io::IMAGE_DISPATCH_LAYER_TEMP_LOCAL_CACHE, aio_comp, offset,
...
template <typename I>
void ImageTempLocalWriteback<I>::aio_flush(io::FlushSource flush_source,
                                  Context *on_finish) {
  CephContext *cct = m_image_ctx.cct;
...
template <typename I>
void ImageTempLocalWriteback<I>::aio_writesame(uint64_t offset, uint64_t length,
                                      ceph::bufferlist&& bl,
                                      int fadvise_flags, Context *on_finish) {
  CephContext *cct = m_image_ctx.cct;
  ldout(cct, 20) << "offset=" << offset << ", "
...
template <typename I>
void ImageTempLocalWriteback<I>::aio_compare_and_write(Extents &&image_extents,
...
template class librbd::cache::ImageTempLocalWriteback<librbd::ImageCtx>;
